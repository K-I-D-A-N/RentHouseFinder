...
export const AuthProvider = ({ children }) => {
  ...
  const updateAuthFields = async (data = {}) => {
    ...
    const checkoutUrl = data.checkout_url || data.checkoutUrl || source.checkout_url || source.checkoutUrl || null;
    const amount = data.amount ?? source.amount ?? null;
    if (txId) setPendingTransactionId(txId);
    if (checkoutUrl) setPendingCheckoutUrl(checkoutUrl);
  ...
  const clearRegistrationDraft = useCallback(() => setRegistrationDraft(null), []);

  const saveRegistrationDraft = useCallback((payload) => {
