...
export default function PropertyDetailScreen({ route, navigation }) {
  ...
  const { role, canViewPremiumListings, isPremiumCustomer } = useAuth();
  ...
  const startPremiumUpgrade = useCallback(() => {
    navigation.navigate("Profile", {
      screen: "CustomerPremiumPaymentScreen",
    });
  }, [navigation]);
  ...
  const fetchProperty = useCallback(async () => {
    ...
    try {
      ...
      const canAccessPremium = canViewPremiumListings || isPremiumCustomer;
      if (propertyData?.requires_premium && !canAccessPremium) {
  ...
  }, [slug, id, canViewPremiumListings, isPremiumCustomer]);
    ...
    return (
      <View style={[styles.centered, { backgroundColor: colors.background, padding: 24 }]}>
        ...
        <Text style={[styles.errorText, { color: colors.text, marginTop: 16, textAlign: "center" }]}>
          {t("propertyDetail.premiumLocked")}
        </Text>
        ...
        <TouchableOpacity
          ...
          onPress={startPremiumUpgrade}
        >
          <Text style={styles.bookButtonText}>{t("propertyDetail.upgradeToPremium")}</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  const canAccessPremium = canViewPremiumListings || isPremiumCustomer;
  const isPremiumLocked = property?.requires_premium === true && isCustomer && !canAccessPremium;
