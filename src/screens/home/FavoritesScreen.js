...
export default function FavoritesScreen({ navigation }) {
  ...
  const favorites = useMemo(() => {
    const favorited = allProperties.filter((p) => favoriteIds.has(String(p.id || p._id || p.pk || "")));
    return filterListingsForCustomer(favorited, user);
  }, [allProperties, favoriteIds, user]);
