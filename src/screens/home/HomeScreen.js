...
import { filterListingsForCustomer } from "../../utils/premiumHelpers";
  ...
  const filteredProperties = useMemo(() => {
    const categoryFiltered =
      selectedCategory === "All"
        ? properties
        : properties.filter(
            (item) => item.category_name?.trim().toLowerCase() === selectedCategory?.trim().toLowerCase()
          );
    return filterListingsForCustomer(categoryFiltered, user);
  }, [properties, selectedCategory, user]);
