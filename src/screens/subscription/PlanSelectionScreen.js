...
export default function PlanSelectionScreen({ navigation, route }) {
  ...
  const { refreshCurrentUser, applySession, register, clearRegistrationDraft } = useAuth();
  const { registrationData, isRegistrationFlow } = route.params || {};

  const navigateToCheckout = (result) => {
    ...
    if (checkoutUrl || txRef || accountStatus === "pending_payment") {
      navigation.replace("CustomerPremiumPaymentScreen", {
  ...
  const handleSelectPlan = async (planId) => {
    try {
      if (isRegistrationFlow && registrationData) {
        const result = await register({ ...registrationData, plan_id: planId });
        ...
        // The user is registered, their plan config might be pending payment.
        // We now route them to select their payment method page.
        navigation.replace("CustomerPremiumPaymentScreen", {
          plan_id: planId,
          paymentType: "landlord_subscription",
        });
